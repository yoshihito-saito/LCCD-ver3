function Sub_PlotNoisySignal(Time, dF_F, noise_variance, options)
    ...
    exportgraphics(gcf, fullfile(options.procs.path{5},'Noise_var_artifact.pdf'), 'Resolution',300);
    ...
    box off
    title('Cell signal')
    exportgraphics(gcf, fullfile(options.procs.path{5},'Noise_var_signal.pdf'), 'Resolution',300);
