...
pnam = '../1_make_data/result';
flis = dir(fullfile(pnam, '*.h5'));
y = zeros(2048,2048);
for ix=1:length(flis)
    fnam = fullfile(pnam, sprintf('IMG_V%d.h5', ix))
    ytmp = h5read(fnam,'/mov');
    y = cat(3, y, ytmp);
end
y(:,:,1) = [];
...
%{
flis = {flis.name};
y = uint16(zeros(512, 512, 8000));
y = zeros(2048,2048,'uint16');
base_ix = 1:500;
for ix=1:3 %length(flis)
    fnam = fullfile(pnam, sprintf('Stablized_%d.tif', ix))
    ytmp = loadtiff(fnam);
    y = cat(3, y, ytmp);
end
y(:,:,1) = [];
...
options_rigid = NoRMCorreSetParms(...
                   ...
                   'output_type','hdf5', 'mem_batch_size', 250,...
                   'us_fac',50);
