%clear
%gcp;
addpath(genpath('~/sdb/node03/matlabLib/NoRMCorre-master'));

im_siz = [2048,2048];
frames_h5 = 1000;

pnam = '../1_make_data/data/Exp01_C1_original';
fbas = 'Exp01_C1-%02d.tiff';

im_enum = prod(im_siz);
M1_a = zeros(im_siz);

for ix=0:16 %length(flis)
    ...
    options_rigid = NoRMCorreSetParms(...
                       ...
                       'output_type','hdf5', ...
                       'h5_filename',sprintf('./result/IMG_V%d.h5',ix+1), ...
                       'mem_batch_size', 250,...
                       'us_fac',50);

    fnam = fullfile(pnam, sprintf(fbas, ix))
    y = loadtiff(fnam);
...
%M1_a(:,:,1) = [];
save('./result/info.mat', 'shifts1a', 'template1');
...
for ix = 1:length(flis)
    createTiff('../results', ...
        sprintf('Stab_nonrig_%02d.tif', ix), ...
        M2(:,:,base_ix + (ix-1)*500));
end
...
% info=h5info(M1);
% siz=info.Datasets.Dataspace.Size;
% x=h5read(M1, '/mov',[1,1,500],[siz(1),siz(2),1]);